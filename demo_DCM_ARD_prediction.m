...
Y_onehot_train = full(ind2vec(Ytr', nChoices))';
Y_onehot_test = full(ind2vec(Yts', nChoices))';
[llik_full,~] = neglog_DCM(mu, X, Y, Y_onehot, availableChoices);
[llik_train,~] = neglog_DCM(mu, Xtr, Ytr, Y_onehot_train, availableChoicesTr);
[llik_test,~] = neglog_DCM(mu, Xts, Yts, Y_onehot_test, availableChoicesTs);
